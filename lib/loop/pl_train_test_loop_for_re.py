...
from lib.models import BERT_TF_REL, compute_loss
...
def batch_processing(model, sentence, tag, batch_re, device):
    ...
    # 損失の計算
    rel_loss = compute_loss(rel_logits, batch_re, device)
...
def train_val_loop_re(train_vecs, ner_train_labels, re_train_gold_labels, 
    ...
    for epoch in range(hyper.max_epoch):
        ...
        # 保存
        logger.info("検証")
        logger.info("{0}エポック目のREの損失値: {1:.4}".format(epoch, val_re_running_loss))

        logger.info("{0}エポック目のREのMicro Avg: {1:.4}".format(epoch, rel_res["micro avg"]["f1-score"]))
        logger.info("{0}エポック目の平均F1: {1:.4}\n".format(epoch, val_F))
        # Early Stopping
        if val_F > best_val_F:
            logger.info("{0}エポック目で更新\n".format(epoch))
            torch.save(model.module.state_dict(), 
                           './models/{0}/{1}/RE/re_model_{2}.pt'.format(hyper.task, hyper.exp_name, fold))
            best_val_F = val_F
        else:
...
def test_loop_re(X_test, test_vecs, ner_test_labels, re_test_gold_labels, 
    ...
    with torch.inference_mode():
        ...
        model.load_state_dict(torch.load('./models/{0}/{1}/RE/re_model_{2}.pt'.format(hyper.task, hyper.exp_name, fold)))
        model = torch.nn.DataParallel(model)
        # 勾配を更新しない
        model.eval()
        # 結果
    ...
    rel_res = evaluate_rel_v2(res_df, rel2idx)
    return res_df, rel_res